//! Isolated 5-Stage Pipeline Latency Benchmark
//!
//! This is a completely standalone benchmark for ONLY 5-stage pipelines.
...
/// Run a single 5-stage pipeline test
async fn run_5_stage_pipeline() -> anyhow::Result<Duration> {
    let temp_dir = tempdir()?;
    let journal_path = temp_dir.path().join(format!("five_stage_{}", std::time::SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_nanos()));
    std::fs::create_dir_all(&journal_path)?;
    
    let journal = Arc::new(DiskJournal::new(journal_path, "benchmark_5_stage").await?);
...
fn bench_5_stage_latency(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();
    let mut group = c.benchmark_group("5_stage_latency");

    group.sample_size(10);  // Minimum required by Criterion
    group.measurement_time(Duration::from_secs(45));  // Increased measurement time for 5+ stages
    ...
    group.bench_function("median_latency", |b| {
        b.to_async(&rt).iter_custom(|iters| async move {
            ...
            for _ in 0..iters {
                let median = run_5_stage_pipeline().await.unwrap();
...
criterion_group!(benches, bench_5_stage_latency);
