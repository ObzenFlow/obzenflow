...
async fn run_3_stage_pipeline() -> anyhow::Result<Duration> {
    ...
    let journal_path = temp_dir.path().join(format!("three_stage_{}", std::time::SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_nanos()));
    std::fs::create_dir_all(&journal_path)?;
    
    let journal = Arc::new(DiskJournal::new(journal_path, "benchmark_3_stage").await?);
