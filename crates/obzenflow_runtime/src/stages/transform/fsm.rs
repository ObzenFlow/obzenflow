...
use crate::stages::resources_builder::{BoundSubscriptionFactory, DirectFactPlan};
    ...
    async fn execute(&self, ctx: &mut Self::Context) -> Result<(), obzenflow_fsm::FsmError> {
        match self {
            ...
            TransformAction::ForwardEOF => {
                ...
                // Preserve causal metadata from the buffered EOF (if any) but
                // always emit an EOF authored by this stage with this stage's
                // own committed Data frontier.
