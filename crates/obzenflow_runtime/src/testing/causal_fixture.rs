...
use obzenflow_core::{FlowId, JournalId, StageId};
...
pub(crate) fn coordinate(label: &str) -> CausalCoordinate {
    ...
    CausalCoordinate::new(
        JournalId::from(ulid::Ulid::from(1_u128)).into(),
        StageId::from(ulid::Ulid::from(value)).into(),
    )
}
...
pub(crate) fn commit<T: JournalEvent>(
    ...
    let previous = records
        .iter()
        .rev()
        .find(|record| record.writer_id() == event.writer_id())
        .map(CausalCommit::from_record)
        .transpose()?;
    let (commitment, causal) = CausalCommit::prepare(
        run_id,
        CausalCoordinate::new(id.into(), *event.writer_id()),
        *event.id(),
        previous.as_ref(),
        &options.frontier,
    )?;
