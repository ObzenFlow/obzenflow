...
//! In-memory journal fixture with readers that observe subsequent appends.

use async_trait::async_trait;
use obzenflow_core::event::{JournalEvent, JournalWriterId};
...
pub(crate) struct MemoryJournal<T: JournalEvent> {
    id: JournalId,
...
impl<T: JournalEvent> MemoryJournal<T> {
    pub(crate) fn with_owner(owner: JournalOwner) -> Self {
        Self {
            id: JournalId::new(),
            owner: Some(owner),
