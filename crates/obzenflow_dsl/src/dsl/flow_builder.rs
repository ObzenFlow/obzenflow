...
//! Ordinary flow materialisation used by the exported authoring macros.
//!
//! Public only for downstream macro reachability. This is not a supported
...
#[doc(hidden)]
pub struct FlowBuildOutput {
    handle: FlowHandle,
    #[cfg_attr(not(feature = "test-support"), allow(dead_code))]
    stage_data_journals: Vec<(StageId, Arc<dyn Journal<ChainEvent>>)>,
}

impl FlowBuildOutput {
    #[doc(hidden)]
    pub fn into_handle(self) -> FlowHandle {
        self.handle
    }
    ...
    pub fn into_test_harness(
        ...
        obzenflow_runtime::testing::FlowTestHarness::from_parts(
            self.handle,
            self.stage_data_journals,
        )
        .map_err(|error| {
            crate::dsl::FlowBuildError::StageResourcesFailed(format!(
                "Failed to build FlowTestHarness: {error}"
            ))
            .into()
        })
    }
    ...
    P: FnOnce(FlowId) -> Result<J, JournalError> + Send,
{
    ...
    fn to_topology_stage_type(
        ...
        match core_stage_type {
            ...
            obzenflow_core::event::context::StageType::Sink => obzenflow_topology::StageType::Sink,
            obzenflow_core::event::context::StageType::Stateful => {
                obzenflow_topology::StageType::Stateful
            }
            obzenflow_core::event::context::StageType::Join => obzenflow_topology::StageType::Join,
        }
    }
    ...
    let __build_result: Result<_, FlowBuildError> = async {
        ...
        let __flow_backpressure_clause = flow_backpressure_clause;
