...
use obzenflow_runtime::stages::common::handlers::{
    ...
    TypedInfiniteSourceHandler, TypedSinkConsumeReport, TypedSinkHandler, TypedTransformHandler,
};
...
impl TypedTransformHandler for MergeTransform {
    type Input = ChannelTick;
    type Output = Merged;

    fn process(&self, tick: ChannelTick) -> Result<Merged, HandlerError> {
        Ok(Merged {
...
impl EffectfulTransformHandler for EffectfulTail {
    type Input = Merged;
    type Output = obzenflow_core::stage_fact_set![AbortOutput, EffectValue];
