...
async fn test_journal_parity() {
    ...
    for journal in [&disk_journal, &memory_journal] {
        ...
        let envelope1 = journal
            .append(event1.clone(), Default::default())
            .await
            .unwrap();
        ...
        let _envelope3 = journal
            .append(event3.clone(), Default::default())
            .await
            .unwrap();
...
async fn test_journal_physical_order_precedes_event_id_across_authors() {
    ...
    for journal in [&disk_journal, &memory_journal] {
        ...
        journal
            .append(low.clone(), Default::default())
            .await
            .unwrap();
...
async fn test_read_causally_after_matches_slice_across_event_authors() {
    ...
    for journal in [&disk_journal, &memory_journal] {
        let env_a = journal
            .append(event_a.clone(), Default::default())
            .await
            .unwrap();
        let _env_b = journal
            .append(event_b.clone(), Default::default())
            .await
            .unwrap();
        let _env_c = journal
            .append(
...
async fn discarded_command_facts_survive_disk_reopen_and_json_round_trip() {
    ...
    for (command, disposition, error) in [
        ...
        disk.append(event.clone(), Default::default())
            .await
            .unwrap();
