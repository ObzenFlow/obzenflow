...
use crate::{FsmBuilder, StateVariant, EventVariant, Transition};
use std::fmt::Debug;
...
fn test_valid_fsm_compiles() {
    let _ = FsmBuilder::new(ValidState::A)
        .when("A")
        .on("Go", |_state: &ValidState, _event: &ValidEvent, _ctx: &mut ValidContext| async {
            Ok(Transition {
...
fn test_wrong_state_type_in_handler() {
    ...
    let _ = FsmBuilder::new(ValidState::A)
        .when("A")
        .on("Go", |_state: &OtherState, _event: &ValidEvent, _ctx: &mut ValidContext| async {
            Ok(Transition {
                next_state: ValidState::B,
                actions: vec![],
            })
...
fn test_wrong_event_type_in_handler() {
    ...
    let _ = FsmBuilder::new(ValidState::A)
        .when("A")
        .on("Go", |_state: &ValidState, _event: &OtherEvent, _ctx: &mut ValidContext| async {
            Ok(Transition {
                next_state: ValidState::B,
                actions: vec![],
            })
...
fn test_wrong_context_type_in_handler() {
    ...
    let _ = FsmBuilder::new(ValidState::A)
        .when("A")
        .on("Go", |_state: &ValidState, _event: &ValidEvent, _ctx: &mut OtherContext| async {
            Ok(Transition {
                next_state: ValidState::B,
                actions: vec![],
            })
...
fn test_cannot_mutate_state_directly() {
    let fsm = FsmBuilder::new(ValidState::A)
        .when("A")
        .on("Go", |state: &ValidState, _event: &ValidEvent, _ctx: &mut ValidContext| async {
...
fn test_phantom_type_safety() {
    ...
    .when("State")
    .on("Login", |state, event, _ctx| async move {
...
async fn test_send_sync_requirements() {
    ...
    // Context can contain !Send types as long as we don't move it
    struct ContextWithRc {
        local_data: Rc<String>,
        shared_data: Arc<String>,
    }
    
    let fsm = FsmBuilder::new(SendState {
        shared_data: Arc::new("initial".to_string()),
    })
    .when("SendState")
    .on("Go", |state: &SendState, _event: &ValidEvent, ctx: &mut ContextWithRc| {
        let shared = state.shared_data.clone();
        let ctx_shared = ctx.shared_data.clone();
        // We can't move ctx.local_data because Rc is !Send
    ...
    let mut machine = fsm;
    let mut ctx = ContextWithRc {
        local_data: Rc::new("local".to_string()),
...
fn test_const_generic_states() {
    ...
    .when("Empty")
    .on("Fill", |_state, _event, _ctx| async {
        Ok(Transition {
    ...
    .done()
    .when("Array")
    .on("Clear", |_state, _event, _ctx| async {
        Ok(Transition {
