...
async fn test_deeply_nested_state() {
    ...
    let fsm = FsmBuilder::new(NestedState::Level1 { data: initial_data })
        .when("Level1")
                    ...
                    for (_key, values) in data {
                        for opt in values {
                            if let Some(inner) = opt {
                                matrix.push(vec![inner.values.clone()]);
                            }
                        }
                    }
            ...
            .done()
        .when("Level2")
            .on("Mutate", |state, event, _ctx| {
                let metadata = match state {
                    NestedState::Level2 { metadata, .. } => metadata.clone(),
                    _ => unreachable!(),
                };
                
                let (key, value) = match event {
                    NestedEvent::Mutate { key, value } => (key.clone(), value.clone()),
                    _ => unreachable!(),
                };
                
                async move {
                    // Mutate shared state
                    metadata.write().await.insert(key, value);
                    
                    // State remains the same, just internal mutation
                    Ok(Transition {
    ...
    // Transform to Level2
    machine.handle(NestedEvent::Transform, &mut ctx).await.unwrap();
    
    // Verify transformation
    if let NestedState::Level2 { matrix, metadata } = machine.state() {
        assert_eq!(matrix.len(), 1);
        assert_eq!(matrix[0].len(), 1);
        assert_eq!(matrix[0][0], vec![1.0, 2.0, 3.0]);
        ...
        assert_eq!(meta.get("test"), Some(&"value".to_string()));
    } else {
...
async fn test_complex_async_handlers() {
    ...
    // Finish
    let actions = machine.handle(AsyncEvent::FinishWork, &mut ctx).await.unwrap();
...
async fn test_state_equality_edge_cases() {
    ...
    // State should not transition if already in target state
    machine.current_state = FloatState::WithNan { value: NAN };
    let actions = machine.handle(FloatEvent::Normalize, &mut ctx).await.unwrap();
...
async fn test_many_states() {
    ...
    let mut builder = FsmBuilder::new(ManyStates::State0);
