...
- **Studio updates.** `studio::StudioProjection` turns committed journal facts into the lifecycle, contract and middleware messages Studio displays. Start with `studio/messages.rs` for the message vocabulary. Replay and live delivery share one state fold; Infra's `StudioUpdatesEndpoint` owns reading and connection lifetime.
