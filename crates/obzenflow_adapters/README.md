...
- **Metrics reporting.** An owned latest-snapshot read model supports Prometheus exposition. Runtime publishes through Core’s `MetricsSnapshotSink`; Infra owns reporting selection, collection lifecycle, and HTTP delivery. Reporting is opt-in; terminal lifecycle snapshots remain independent.
