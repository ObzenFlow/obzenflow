...
- **Monitoring projections.** An owned latest-snapshot read model supports Prometheus text and console summaries. Runtime publishes through Core’s `MetricsSnapshotSink`; Infra owns backend selection, HTTP delivery, console output, and shutdown. Monitoring is opt-in.
