...
use crate::middleware::{
    ...
    ControlStrategyRequirement, ErrorAction,
};
...
impl Middleware for WindowingMiddleware {
    fn pre_handle(&self, event: &ChainEvent, ctx: &mut MiddlewareContext) -> MiddlewareAction {
        ...
        if event.is_control() {
            ...
            if event.event_type == ChainEvent::EOF_EVENT_TYPE {
    ...
    fn test_windowing_factory_creates_middleware() {
        ...
        match requirement {
            ControlStrategyRequirement::Windowing { window_duration } => {
                assert_eq!(window_duration, Duration::from_secs(60));
            }
            _ => panic!("Expected windowing requirement"),
        }
    }
