//! Timing middleware for measuring stage processing time
//!
//! This middleware implements the wide events pattern by enriching events
//! with processing duration before they're written to the journal.

use super::{Middleware, MiddlewareAction, ErrorAction, MiddlewareContext, MiddlewareFactory};
use obzenflow_core::event::chain_event::ChainEvent;
use obzenflow_runtime_services::pipeline::config::StageConfig;
use std::time::{SystemTime, UNIX_EPOCH};

/// Middleware that measures processing time and adds it to events
/// 
/// This is a core system middleware that should be added to all stages
/// to provide automatic timing instrumentation.
#[derive(Debug, Clone)]
pub struct TimingMiddleware {
    stage_name: String,
}

impl TimingMiddleware {
    /// Create a new timing middleware for a specific stage
    pub fn new(stage_name: impl Into<String>) -> Self {
        Self {
            stage_name: stage_name.into(),
        }
    }
}

impl Middleware for TimingMiddleware {
    fn pre_handle(&self, _event: &ChainEvent, ctx: &mut MiddlewareContext) -> MiddlewareAction {
        // Record the start time in the context as nanoseconds since epoch
        let start_nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos() as u64;
        ctx.set_baggage("processing_start_nanos", serde_json::json!(start_nanos));
        MiddlewareAction::Continue
    }
    
    fn post_handle(&self, _event: &ChainEvent, _results: &[ChainEvent], _ctx: &mut MiddlewareContext) {
        // Post-handle doesn't need to do anything - timing is added in pre_write
    }
    
    fn on_error(&self, _event: &ChainEvent, _ctx: &mut MiddlewareContext) -> ErrorAction {
        // Just propagate errors - timing middleware doesn't handle errors
        ErrorAction::Propagate
    }
    
    fn pre_write(&self, event: &mut ChainEvent, ctx: &MiddlewareContext) {
        // Calculate processing time and add to event
        if let Some(start_value) = ctx.get_baggage("processing_start_nanos") {
            if let Some(start_nanos) = start_value.as_u64() {
                let now_nanos = SystemTime::now()
                    .duration_since(UNIX_EPOCH)
                    .unwrap()
                    .as_nanos() as u64;
                let duration_nanos = now_nanos - start_nanos;
                let duration_ms = duration_nanos / 1_000_000;
                
                // Add processing time to the event's processing_info
                event.processing_info.processing_time_ms = duration_ms;
                
                // Also add stage-specific timing in payload for debugging
                if let Some(metrics) = event.payload.get_mut("_metrics").and_then(|v| v.as_object_mut()) {
                    metrics.insert(
                        format!("{}_processing_time_ms", self.stage_name),
                        serde_json::json!(duration_ms)
                    );
                } else {
                    // Create metrics object if it doesn't exist
                    event.payload["_metrics"] = serde_json::json!({
                        format!("{}_processing_time_ms", self.stage_name): duration_ms
                    });
                }
                
                tracing::debug!(
                    "TimingMiddleware: Added {}ms processing time to event {} (stage: {}) [{}ns total]", 
                    duration_ms, 
                    event.id,
                    self.stage_name,
                    duration_nanos
                );
                
                // Log warning if timing seems too low for processor stage
                if self.stage_name.contains("processor") && duration_ms < 5 {
                    tracing::warn!(
                        "TimingMiddleware: Processor timing seems too low: {}ms ({}ns) for event {}",
                        duration_ms,
                        duration_nanos,
                        event.id
                    );
                }
            } else {
                tracing::warn!(
                    "TimingMiddleware: Could not parse start time from baggage for event {}", 
                    event.id
                );
            }
        } else {
            tracing::warn!(
                "TimingMiddleware: No processing start time found for event {}", 
                event.id
            );
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use obzenflow_core::{EventId, WriterId};
    use serde_json::json;
    use std::thread;
    use std::time::Duration;
    
    #[test]
    fn test_timing_middleware_adds_processing_time() {
        let middleware = TimingMiddleware::new("test_stage");
        let mut ctx = MiddlewareContext::new();
        
        let event = ChainEvent::new(
            EventId::new(),
            WriterId::new(),
            "test.event",
            json!({"data": "test"})
        );
        
        // Pre-handle starts the timer
        let action = middleware.pre_handle(&event, &mut ctx);
        assert!(matches!(action, MiddlewareAction::Continue));
        
        // Verify start time was recorded
        assert!(ctx.get_baggage("processing_start_nanos").is_some());
        
        // Simulate some processing time
        thread::sleep(Duration::from_millis(10));
        
        // Pre-write should add the timing
        let mut result_event = event.clone();
        middleware.pre_write(&mut result_event, &ctx);
        
        // Check that processing time was added
        assert!(result_event.processing_info.processing_time_ms >= 9); // Allow for timing variance
        assert!(result_event.payload["_metrics"]["test_stage_processing_time_ms"].as_u64().unwrap() >= 9);
    }
    
    #[test]
    fn test_timing_middleware_handles_missing_start_time() {
        let middleware = TimingMiddleware::new("test_stage");
        let ctx = MiddlewareContext::new(); // No start time set
        
        let mut event = ChainEvent::new(
            EventId::new(),
            WriterId::new(),
            "test.event",
            json!({"data": "test"})
        );
        
        // Pre-write should handle missing start time gracefully
        middleware.pre_write(&mut event, &ctx);
        
        // Processing time should remain at default (0)
        assert_eq!(event.processing_info.processing_time_ms, 0);
    }
    
    #[test]
    fn test_timing_middleware_preserves_existing_metrics() {
        let middleware = TimingMiddleware::new("test_stage");
        let mut ctx = MiddlewareContext::new();
        
        let mut event = ChainEvent::new(
            EventId::new(),
            WriterId::new(),
            "test.event",
            json!({
                "data": "test",
                "_metrics": {
                    "existing_metric": 42
                }
            })
        );
        
        // Pre-handle starts the timer
        middleware.pre_handle(&event, &mut ctx);
        
        // Pre-write should add timing without removing existing metrics
        middleware.pre_write(&mut event, &ctx);
        
        // Check both metrics exist
        assert_eq!(event.payload["_metrics"]["existing_metric"], 42);
        assert!(event.payload["_metrics"]["test_stage_processing_time_ms"].is_number());
    }
}

/// Factory for creating TimingMiddleware instances with stage context
pub struct TimingMiddlewareFactory;

impl TimingMiddlewareFactory {
    pub fn new() -> Self {
        Self
    }
}

impl MiddlewareFactory for TimingMiddlewareFactory {
    fn create(&self, config: &StageConfig) -> Box<dyn Middleware> {
        Box::new(TimingMiddleware::new(&config.name))
    }
    
    fn name(&self) -> &str {
        "timing"
    }
}

/// Convenience function to create a timing middleware factory
pub fn timing() -> Box<dyn MiddlewareFactory> {
    Box::new(TimingMiddlewareFactory::new())
}