...
impl Middleware for SystemEnrichmentMiddleware {
    ...
    fn pre_write(&self, event: &mut ChainEvent, _ctx: &MiddlewareContext) {
        ...
        if !event.is_control() {  // Don't add correlation to control events
            match self.stage_type {
                ...
                StageType::Transform | StageType::Sink => {
                    // Transforms and sinks preserve correlation IDs
                    // If somehow we get here without a correlation ID, log a warning
                    if event.correlation_id.is_none() && !event.is_control() {
                        tracing::warn!(
        ...
        // Also enrich control events with proper context (but no correlation)
        if event.is_control() {
...
mod tests {
    ...
    fn test_control_events_no_correlation() {
        let middleware = SystemEnrichmentMiddleware::new(
            ...
            "flow-789",
            "rate_limiter_source",
            StageType::Source  // Even sources shouldn't add correlation to control events
        );
        let ctx = MiddlewareContext::new();
        
        let mut event = ChainEvent::control(
