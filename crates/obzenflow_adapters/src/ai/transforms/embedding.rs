...
use crate::ai::{effect_error_to_handler_error, EmbeddingEffects, EmbeddingGeneration};
...
/// Typed standalone embedding handler using the declaration-scoped embedding operation.
    ...
    Out: TypedPayload + Send + Sync + 'static,
{
    ...
    async fn process(
        ...
        let reply = fx
            .generate_embedding(STANDALONE_EMBEDDING_GENERATION_LABEL, spec)
            .await
