...
impl UtilizationMetric {
    ...
    pub fn reset(&self) {
        self.gauge.reset();
        self.current_state
            .store(UtilizationState::Waiting as u64, Ordering::Relaxed);
        self.last_change
            .store(self.uptime_nanos(), Ordering::Relaxed);
    ...
    fn transition_to(&self, new_state: UtilizationState) {
        ...
        self.current_state
            .store(new_state as u64, Ordering::Relaxed);
...
mod tests {
    ...
    async fn test_utilization_support_trait() {
        ...
        assert_eq!(
            stage.utilization_metric().current_state(),
            UtilizationState::Working
        );
