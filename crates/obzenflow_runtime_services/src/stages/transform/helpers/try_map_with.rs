...
//!
//! ```rust
//! use obzenflow_runtime_services::stages::transform::helpers::TryMapWith;
...
pub enum ErrorStrategy {
    ...
    /// })
    /// ```
    ToEventTypeWith(String, Arc<dyn Fn(&ChainEvent, &str) -> serde_json::Value + Send + Sync>),
...
impl std::fmt::Debug for ErrorStrategy {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ErrorStrategy::ToEventTypeWith(event_type, _) => write!(f, "ToEventTypeWith({}, <closure>)", event_type),
    ...
    F: Fn(ChainEvent) -> Result<ChainEvent, String> + Send + Sync + Clone,
{
    ...
    {
        self.error_strategy = ErrorStrategy::ToEventTypeWith(event_type.into(), Arc::new(payload_fn));
        self
    }
...
mod tests {
    ...
    async fn test_typed_try_map_with_deserialization_failure() {
        ...
        let validator = TryMapWithTyped::new(
            |input: TestInput| -> Result<TestOutput, String> {
                Ok(TestOutput {
                    value: input.value,
                    name: input.name,
                    validated: true,
    ...
    async fn test_typed_try_map_with_on_error_emit_with() {
        ...
        .on_error_emit_with("TestInput.invalid", |_event, error| {
            json!({"error_details": {"message": error, "severity": "high"}})
        });
