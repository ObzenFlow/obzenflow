...
use super::fsm::{
    JournalSinkState, JournalSinkEvent, JournalSinkAction,
    JournalSinkContext,
};

/// Supervisor for journal sink stages
pub(crate) struct JournalSinkSupervisor<H: SinkHandler + Clone + std::fmt::Debug + Send + Sync + 'static> {
...
impl<H: SinkHandler + Clone + std::fmt::Debug + Send + Sync + 'static> crate::supervised_base::base::private::Sealed for JournalSinkSupervisor<H> {}

impl<H: SinkHandler + Clone + std::fmt::Debug + Send + Sync + 'static> Supervisor for JournalSinkSupervisor<H> {
...
impl<H: SinkHandler + Clone + std::fmt::Debug + Send + Sync + 'static> HandlerSupervised for JournalSinkSupervisor<H> {
    type Handler = H;
    ...
    async fn dispatch_state(
        ...
        match state {
            JournalSinkState::Created => {
                ...
                Ok(EventLoopDirective::Transition(JournalSinkEvent::Initialize))
            }
                    ...
                    match subscription.recv().await {
                        Ok(envelope) => {
                            ...
                            if envelope.event.is_eof() {
                                ...
                                return Ok(EventLoopDirective::Transition(JournalSinkEvent::ReceivedEOF));
                            }
            ...
            JournalSinkState::Flushing => {
                ...
                Ok(EventLoopDirective::Transition(JournalSinkEvent::FlushComplete))
            }
