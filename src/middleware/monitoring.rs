    ...
    T::Metrics: MetricRecorder,
{
    fn pre_handle(&self, event: &ChainEvent) -> MiddlewareAction {
        ...
        EVENT_START_TIMES.with(|times| {
            times.borrow_mut().insert(event.ulid, start_time);
        });
    ...
    fn on_error(&self, event: &ChainEvent, _error: &super::StepError) -> super::ErrorAction {
        ...
        EVENT_START_TIMES.with(|times| {
            times.borrow_mut().remove(&event.ulid);
        });
...
use crate::event_types::ProcessingOutcome;
...
impl MetricRecorder for USEMetrics {
    ...
    fn on_event_complete(&self, event: &ChainEvent, _results: &[ChainEvent], _duration: Duration) {
        ...
        // Check for errors
        if let ProcessingOutcome::Failed { .. } = &event.processing_info.outcome {
            self.record_error();
        }
    }
...
impl MetricRecorder for SAAFEMetrics {
    ...
    fn on_event_complete(&self, event: &ChainEvent, _results: &[ChainEvent], _duration: Duration) {
        ...
        match &event.processing_info.outcome {
            ProcessingOutcome::Succeeded { quality_score, .. } => {
                // If quality is degraded, might be an anomaly
                if let Some(quality) = quality_score {
                    if *quality < 0.8 {
                        self.record_anomaly(1.0 - quality);
                    }
                }
            }
            ProcessingOutcome::Failed { .. } => {
