...
pub fn timeout(duration: Duration) -> Box<dyn Middleware> {
    Box::new(middleware_fn(move |event| {
        ...
        tracing::debug!("Timeout middleware: {:?} for event {}", duration, event.ulid);
        MiddlewareAction::Continue
    }))
}
...
pub fn logging(level: tracing::Level) -> Box<dyn Middleware> {
    Box::new(middleware_fn(move |event| {
        match level {
            ...
            tracing::Level::DEBUG => tracing::debug!("Processing event: {}", event.ulid),
            tracing::Level::INFO => tracing::info!("Processing event: {} ({})", event.ulid, event.event_type),
            tracing::Level::WARN => tracing::warn!("Processing event: {}", event.ulid),
            tracing::Level::ERROR => tracing::error!("Processing event: {}", event.ulid),
        }
        MiddlewareAction::Continue
    }))
}
...
pub mod retry {
    ...
    impl ExponentialRetryBuilder {
        ...
        pub fn build(self) -> Box<dyn Middleware> {
            ...
            Box::new(middleware_fn(move |event| {
                tracing::debug!("Retry middleware configured for event {}", event.ulid);
                MiddlewareAction::Continue
            }))
        }
    }
            ...
            Box::new(middleware_fn(move |event| {
                let msg = if self.with_payload {
                    format!("Processing event: {} ({}) - payload: {:?}", 
                        event.ulid, event.event_type, event.payload)
                } else {
                    format!("Processing event: {} ({})", 
                        event.ulid, event.event_type)
                };
