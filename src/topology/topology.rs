...
use crate::topology::{StageId, StageInfo, DirectedEdge, TopologyError};

/// Complete pipeline topology with efficient traversal
#[derive(Debug, Clone)]
...
impl PipelineTopology {
    ...
    pub fn new(stages: Vec<StageInfo>, edges: Vec<DirectedEdge>) -> Result<Self, TopologyError> {
        ...
        crate::topology::validation::validate_acyclic(&stage_map, &downstream)?;
        
        // Detect disconnected components
        if let Some(disconnected) = crate::topology::validation::find_disconnected_stages(&stage_map, &downstream, &upstream) {
