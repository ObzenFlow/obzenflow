...
async fn test_rate_limiter_no_event_loss() {
    ...
    assert!(elapsed >= Duration::from_secs(9), "Should take at least 9 seconds");
    assert!(elapsed <= Duration::from_secs(12), "Should take no more than 12 seconds");
}
...
async fn test_rate_limiter_with_burst() {
    ...
    assert!(elapsed >= Duration::from_secs(2), "Should take at least 2 seconds");
    assert!(elapsed <= Duration::from_secs(5), "Should take no more than 5 seconds");
}
...
async fn test_rate_limiter_control_events_not_blocked() {
    ...
    assert!(elapsed <= Duration::from_secs(6), "EOF should not be blocked by rate limiter");
