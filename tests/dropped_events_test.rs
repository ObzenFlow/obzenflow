...
use obzenflow_core::{
    event::{chain_event::ChainEvent, event_id::EventId, correlation::CorrelationId},
...
async fn test_dropped_events_detection() -> Result<()> {
    ...
    assert_eq!(events.len(), 7, "Should have received 7 events (10 - 3 dropped)");
    
    // Debug: Check if events have correlation IDs
    println!("\n=== Debug: Event Correlation IDs ===");
    for (i, event) in events.iter().enumerate() {
        println!("Event {}: correlation_id = {:?}", i, event.correlation_id.is_some());
    }
    ...
    if let Some(line) = metrics_text.lines().find(|l| l.contains("obzenflow_dropped_events{flow=\"correlation_test_flow\"")) {
