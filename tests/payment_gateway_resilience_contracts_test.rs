...
//! - Strict flow (no circuit breaker) records failed gateway effects on the
//!   error rail and completes without transport divergence.
...
fn payment_gateway_strict_flow_completes_without_transport_divergence() {
    let result = payment_gateway_resilience::flow::run_strict_example_in_tests();
    assert!(
        result.is_ok(),
        "strict payment gateway flow should record failed effects without transport divergence"
    );
}
