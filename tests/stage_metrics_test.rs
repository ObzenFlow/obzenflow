...
async fn test_stage_level_metrics_automatic() -> Result<()> {
    ...
    let has_source_events = metrics_text.contains(r#"obzenflow_events_total{flow="stage_metrics_test",stage="test_source"}"#);
    let has_transform_events = metrics_text.contains(r#"obzenflow_events_total{flow="stage_metrics_test",stage="uppercase_transform"}"#);
    let has_sink_events = metrics_text.contains(r#"obzenflow_events_total{flow="stage_metrics_test",stage="collector_sink"}"#);
    
    println!("Source events_total: {}", if has_source_events { "✓" } else { "✗" });
    println!("Transform events_total: {}", if has_transform_events { "✓" } else { "✗" });
    println!("Sink events_total: {}", if has_sink_events { "✓" } else { "✗" });
    
    // Check duration_seconds histograms
    let has_source_duration = metrics_text.contains(r#"obzenflow_duration_seconds_count{flow="stage_metrics_test",stage="test_source"}"#);
    ...
    let has_sink_duration = metrics_text.contains(r#"obzenflow_duration_seconds_count{flow="stage_metrics_test",stage="collector_sink"}"#);
    
    println!("\nSource duration histogram: {}", if has_source_duration { "✓" } else { "✗" });
    println!("Transform duration histogram: {}", if has_transform_duration { "✓" } else { "✗" });
    println!("Sink duration histogram: {}", if has_sink_duration { "✓" } else { "✗" });
    ...
    assert!(has_source_events, "Source stage MUST have events_total metric");
    assert!(has_transform_events, "Transform stage MUST have events_total metric");
    assert!(has_sink_events, "Sink stage MUST have events_total metric");
    
    assert!(has_source_duration, "Source stage MUST have duration_seconds metric");
    assert!(has_transform_duration, "Transform stage MUST have duration_seconds metric");
    assert!(has_sink_duration, "Sink stage MUST have duration_seconds metric");
    
    // Check that flow context was populated correctly
    println!("\n=== Verifying Flow Context ===");
    for (i, event) in events.iter().enumerate() {
        println!("Event {}: flow={}, stage={}", 
            i + 1,
            event.flow_context.flow_name,
            event.flow_context.stage_name
        );
        assert_ne!(event.flow_context.flow_name, "unknown", "Flow name must be populated");
        assert_ne!(event.flow_context.stage_name, "unknown", "Stage name must be populated");
    }
