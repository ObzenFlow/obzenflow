...
async fn test_notify_race_condition() {
    ...
    let result = tokio::time::timeout(tokio::time::Duration::from_millis(100), notified).await;
    ...
    let result2 = tokio::time::timeout(tokio::time::Duration::from_millis(100), notified2).await;
