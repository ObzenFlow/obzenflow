...
use obzenflow_runtime_services::stages::common::handlers::{
    SinkHandler, SourceHandler, TransformHandler,
};
use serde_json::json;
...
impl SourceHandler for BurstSource {
    async fn run(
        &mut self,
        emitter: obzenflow_runtime_services::EventEmitter,
    ) -> obzenflow_core::Result<()> {
        println!(
            "Starting burst source: {} bursts of {} events each",
            self.burst_size, self.events_per_burst
        );

        for burst in 0..self.burst_size {
            let burst_start = Instant::now();
            println!(
                "\nBurst {}: Emitting {} events rapidly",
                burst + 1,
                self.events_per_burst
            );
...
impl TransformHandler for TimestampTransform {
    fn process(&self, mut event: ChainEvent) -> Vec<ChainEvent> {
        event.payload["processed_at"] = json!(std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_millis());
        vec![event]
...
async fn test_rate_limiter_metrics_end_to_end() -> Result<()> {
    ...
    if has_rl_delay_rate {
        let delay_lines: Vec<&str> = metrics_text
            .lines()
        ...
        let has_delays = delay_lines.iter().any(|line| !line.ends_with("} 0"));
    ...
    if has_rl_utilization {
        let util_lines: Vec<&str> = metrics_text
            .lines()
        ...
        let high_utilization = util_lines.iter().any(|line| {
        ...
        assert!(
            high_utilization,
            "Rate limiter should show high utilization during bursts"
        );
    }
