...
//! Architecture and erasure-boundary guards for FLOWIP-134h.
...
impl<'ast> Visit<'ast> for SurfaceVisitor {
    ...
    fn visit_item_impl(&mut self, implementation: &'ast syn::ItemImpl) {
        let implements_raw_sink = implementation
            .trait_
            .as_ref()
            .and_then(|(_, path, _)| path.segments.last())
            .is_some_and(|segment| segment.ident == "SinkHandler");
        if implements_raw_sink {
...
#[test]
fn retired_sink_surface_stays_absent() {
    ...
    assert!(
        violations.is_empty(),
        "retired sink identifiers resurfaced:\n{}",
        violations.join("\n")
    );
}

#[test]
fn raw_sink_implementations_match_the_checked_in_allowlist() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    ...
    for source_path in workspace_rust_sources(&root) {
        ...
        for implementor in visitor.raw_implementors {
            assert!(
                ...
                "duplicate raw SinkHandler implementation: {relative}|{implementor}"
            );
        }
    }
