...
obzenflow_core::effect_outcome! {
    /// Stateful `Fact` enum generated by the FLOWIP-120m macro: the same sum
    /// shape an effect outcome carrier uses also serves the per-fact Mealy
    /// fold, decoding each committed fact into its matching variant.
    #[derive(Clone, Debug)]
