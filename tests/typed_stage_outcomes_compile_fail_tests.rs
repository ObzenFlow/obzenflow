...
#[test]
fn production_sink_error_authority_fails_closed() {
    let t = trybuild::TestCases::new();
    t.compile_fail("tests/compile_fail/sink_operation_authority/*.rs");
}

#[test]
fn sink_policy_authority_is_admission_and_closed_evidence_only() {
    let t = trybuild::TestCases::new();
    t.compile_fail("tests/compile_fail/sink_policy_authority/*.rs");
}
