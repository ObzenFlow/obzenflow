...
async fn test_middleware_control_events_end_to_end() -> Result<()> {
    ...
    assert!(
        metrics_text.contains("obzenflow_events_total"),
    ...
    assert!(
        metrics_text.contains("obzenflow_duration_seconds"),
    ...
    println!(
        ...
        if has_anomalies {
            ...
            "✗ MISSING (OK - requires anomaly detection)"
        }
    );
    println!(
        "  Amendments total: {}",
        if has_amendments {
            ...
            "✗ MISSING (OK - requires lifecycle events)"
        }
    );
    ...
    assert!(
        has_event_metrics,
        "Expected events_total metrics to be present"
    );
