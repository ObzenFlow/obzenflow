...
#[tokio::test]
async fn test_dsl_pattern_equivalence() -> Result<()> {
    // Verify that named and auto-named DSL patterns produce equivalent behavior
    // This is important for FLOWIP-017 since both patterns share significant code
    
    const EVENTS_PER_PIPELINE: u64 = 15;
    const NUM_RUNS: usize = 5;
    
    let mut named_timings = Vec::new();
    let mut auto_timings = Vec::new();
    
    for run in 0..NUM_RUNS {
        // Test named pattern
        {
            let (store, _temp_dir) = create_test_store_with_uuid(&format!("named_{}", run)).await?;
            let source = TestSource::new(EVENTS_PER_PIPELINE);
            let sink = TestSink::new();
            let sink_clone = sink.clone();
            ...
            let handle = flow! {
                store: store,
                flow_taxonomy: GoldenSignals,
                ("my_source" => source, RED)
                |> ("my_stage" => PassthroughStage::new("my_stage"), USE)
                |> ("my_sink" => sink, RED)
            }?;
            
            let (count, duration) = run_pipeline_timed(handle, sink_clone, EVENTS_PER_PIPELINE).await?;
            assert_eq!(count, EVENTS_PER_PIPELINE);
            named_timings.push(duration);
        }
        
        // Test auto-named pattern
        {
            let (store, _temp_dir) = create_test_store_with_uuid(&format!("auto_{}", run)).await?;
            let source = TestSource::new(EVENTS_PER_PIPELINE);
            let sink = TestSink::new();
            let sink_clone = sink.clone();
            ...
            let handle = flow! {
                ...
                flow_taxonomy: GoldenSignals,
                (source, RED)
                |> (PassthroughStage::new("auto_stage"), USE)
                |> (sink, RED)
            }?;
            
            let (count, duration) = run_pipeline_timed(handle, sink_clone, EVENTS_PER_PIPELINE).await?;
            assert_eq!(count, EVENTS_PER_PIPELINE);
            auto_timings.push(duration);
        }
    }
    
    // Calculate averages
    let named_avg = named_timings.iter().sum::<Duration>() / named_timings.len() as u32;
    let auto_avg = auto_timings.iter().sum::<Duration>() / auto_timings.len() as u32;
    
    println!("Named pattern avg: {:?}", named_avg);
    println!("Auto pattern avg: {:?}", auto_avg);
    
    // Both patterns should have similar performance (within 20%)
    let difference = if named_avg > auto_avg {
        named_avg - auto_avg
    } else {
        auto_avg - named_avg
    };
    ...
    assert!(
        difference < named_avg / 5 && difference < auto_avg / 5,
        "Named and auto patterns should have similar performance (within 20%)"
    );
