...
### Added
- (FLOWIP-115h) Circuit-breaker recovery at protected live-I/O boundaries: declared retry-safe async source polls, idempotent or stably keyed effects, and idempotent sink deliveries can use the existing `CircuitBreakerBuilder::with_retry_fixed` or `with_retry_exponential` spellings. Retry owns one hard total-wall deadline, holds input credit until one terminal outcome, and is bypassed during strict replay.
- (FLOWIP-115h) Typed live retry failures: `SourceError::RateLimited`, `SourceError::PermanentFailure`, `EffectError::TransientExecution`, and `EffectError::RateLimited`. Async source handlers can declare `SourcePollRetrySafety` and `SourcePollRetryOwnership`; the defaults remain source-compatible and fail closed when retry is attached.
...
### Changed
- (FLOWIP-115h) **Behaviour activation**: retry configuration that FLOWIP-115b accepted but deliberately ignored now executes only at eligible source-poll, declared-effect, or sink-delivery boundaries. `max_attempts` counts total physical executions including the initial call. Generic handler-shell and transactional-effect retry remain unsupported.
- (FLOWIP-115h) **Breaking Rust API**: downstream exhaustive matches must handle the new `SourceError` and `EffectError` variants. `RetryEvent` variants gain typed context/exhaustion fields with read-compatible serde defaults, and legacy `Exhausted.last_error` becomes `Option<String>`; prefer framework constructors or use `..` in destructuring patterns.
