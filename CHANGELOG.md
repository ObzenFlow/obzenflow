...
### Added
- (FLOWIP-120m) `effect_outcome!` declarative macro in `obzenflow_core`: define an effect outcome carrier as an enum (closed sum, one persisted fact per variant) or named-field struct (product, one fact per field) and get an exact, fail-closed `TypedFactSet` implementation. Malformed shapes fail at compile time; duplicate member event types are rejected at flow build.
- (FLOWIP-120m) `EffectOutcomeFacts`: public supertrait alias over `TypedFactSet`, now the `Effect::Outcome` bound.
